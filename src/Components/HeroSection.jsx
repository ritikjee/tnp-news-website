...
function HeroSection({ news }) {
  ...
  return (
    ...
    <section className={`relative  flex items-center bg-neutral px-2 py-20`}>
      <button className='btn btn-primary hidden md:flex' onClick={()=>{
        if(page===0){
      ...
      <div className="mx-auto max-w-screen-xl px-4 lg:flex lg:items-center">
        ...
        <div>
          <img src={news[page%news.length].urlToImage||img} alt="no-img" className='w-[500px] h-[500px]' />
        </div>
        </div>
        <div className='flex gap-10 py-10'>
        <button className='btn btn-primary md:hidden ' onClick={()=>{
          if(page===0){
            setPage(news.length-1)
          }
          else{
